...
describe('priority', () => {
  ...
  it('replays high before low — high items complete before low items start', async () => {
    ...
    const highFn = vi.fn().mockImplementation(async () => {
      callOrder.push('high');
    });

    const lowAction = action(lowFn, {
      reliability: 'neverLose',
      name: 'prio-order-low',
      priority: 'low',
    });
    const highAction = action(highFn, {
      reliability: 'neverLose',
      name: 'prio-order-high',
      priority: 'high',
    });
  ...
  it('normal replays before low', async () => {
    ...
    const normalFn = vi.fn().mockImplementation(async () => {
  ...
  it('replayQueue result counts items across tiers', async () => {
    ...
    const succeedAction = action(succeedFn, {
      reliability: 'neverLose',
      name: 'prio-count-succeed',
      priority: 'high',
    });
    const failAction = action(failFn, {
      reliability: 'neverLose',
      name: 'prio-count-fail',
      priority: 'low',
      maxRetries: 1,
    });
