    ...
    <div className="max-w-6xl mx-auto p-5 animate-fade-in">
      <div className="border-b border-eidos-border pb-4 mb-5">
        ...
        <p className="text-xs text-eidos-muted">
          Every <code className="text-eidos-accent">resource()</code> call registers a
          fetch-intercept rule with the service worker. Expand a row to inspect the generated
          caching strategy.
        </p>
      </div>
      ...
      {entries.length === 0 ? (
        <div className="border border-eidos-border p-8 text-center">
          ...
          <p className="text-xs text-eidos-muted">
            call{' '}
            <code className="text-eidos-accent">
              {"resource('/api/url', "}
              {'{ offline: true }'}
              {')'}
            </code>{' '}
...
function ResourceRow({ entry }: { entry: ResourceEntry }) {
  ...
  return (
    <div>
      ...
      >
        <span className="text-eidos-accent font-bold w-4" aria-hidden="true">
          {open ? '▼' : '▶'}
        </span>
        ...
        <span className="text-eidos-accent font-tabular text-2xs w-10 text-right">
          {entry.cacheHits}
        </span>
        ...
        <div className="border-t border-eidos-border bg-eidos-surface px-4 py-4 animate-fade-in">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-4">
            {[
              ...
              [
                'max age',
                entry.config.maxAge ? `${(entry.config.maxAge / 1000).toFixed(0)}s` : '∞',
              ],
          ...
          {/* Reasoning */}
          <div className="mb-4">
            <div className="text-2xs text-eidos-muted uppercase tracking-widest mb-1">
              why this strategy
            </div>
          ...
          {/* Behavior steps */}
          <div>
            <div className="text-2xs text-eidos-muted uppercase tracking-widest mb-2">
              runtime behavior
            </div>
