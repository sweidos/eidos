        ...
        <div className="bg-eidos-bg border border-eidos-border p-3">
          <div className="flex items-center gap-2 text-xs mb-2">
            <code className="text-eidos-muted">
              ...
              {sel.config.strategy ? `, strategy: '${sel.config.strategy}'` : ''}
            </code>
    ...
    <div className="max-w-3xl mx-auto p-5 animate-fade-in">
      <div className="border-b border-eidos-border pb-4 mb-5">
        ...
        <p className="text-xs text-eidos-muted">
          Trace a declaration end-to-end: intent → strategy resolution → SW postMessage → runtime
          behavior.
        </p>
      </div>
        ...
        {STEPS.map(({ n, title, note, content }) => (
          <div key={n} className="p-4">
            <div className="flex items-center gap-2 mb-3">
              ...
              <span className="text-xs font-bold text-eidos-text uppercase tracking-wider">
                {title}
              </span>
      ...
      <div className="mt-5 border border-eidos-border">
        ...
        <div className="divide-y divide-eidos-border">
          {[
            {
              cond: 'offline: true (default)',
              strategy: 'StaleWhileRevalidate',
              sw: 'stale-while-revalidate',
            },
            {
              cond: "offline: true, strategy: 'cache-first'",
              strategy: 'CacheFirst',
              sw: 'cache-first',
            },
            {
              cond: "offline: true, strategy: 'network-first'",
            ...
            return (
              <div
                key={row.sw}
                className={`flex items-center gap-3 px-4 py-2.5 text-xs transition-colors ${active ? 'bg-eidos-accent-dim' : ''}`}
              >
                <code
                  className={`flex-1 text-2xs ${active ? 'text-eidos-text' : 'text-eidos-muted'}`}
                >
                  {row.cond}
                </code>
                <span className="text-eidos-border">→</span>
                <code
                  className={`text-2xs font-bold ${active ? 'text-eidos-accent' : 'text-eidos-muted'}`}
                >
                  {row.strategy}
                </code>
              </div>
